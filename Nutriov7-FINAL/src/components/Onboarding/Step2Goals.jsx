...
import { TrendingDown, TrendingUp, Minus, Calendar, Target } from 'lucide-react';
import { setWeightGoal, setPace, setTargetDate, setTargetWeight } from '../../store/onboardingSlice';

const Step2Goals = () => {
  const dispatch = useDispatch();
  const { weightGoal, pace, targetDate, targetWeight, errors } = useSelector(state => state.onboarding);
  ...
  // Calculate recommended date based on pace and weight goal
  const getRecommendedDate = () => {
    if (!targetWeight || !weightGoal || !pace) return null;
    
    const paceRates = { slow: 0.25, moderate: 0.5, fast: 1 };
    const weeklyRate = paceRates[pace];
    const weeksNeeded = Math.abs(targetWeight) / weeklyRate;
    
    const recommended = new Date();
    recommended.setDate(recommended.getDate() + (weeksNeeded * 7));
  ...
  return (
    <div className="space-y-8">
      ...
      >
        ...
        {targetWeight && pace && weightGoal !== 'maintain' && (
          <p className="mt-2 text-sm text-gray-600">
            💡 Recommended date based on your pace: <strong>{new Date(getRecommendedDate()).toLocaleDateString()}</strong>
          </p>
        )}
      </motion.div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <p className="text-sm text-blue-800">
          <strong>💡 Tip:</strong> Slow and steady wins the race! Gradual changes are more sustainable and healthier.
        </p>
      </div>
    </div>
  );
